<template>
  <div
    v-show="commentModal"
    @click.self="hideCommentsHandler"
    class="comments-bg"
  >
      ...
      <div class="col-1">
        ...
        <form @submit.prevent="submitHandler">
          <textarea
            rows="10"
            :placeholder="
              !userInfo.id ? 'Please Sign In to Comment!' : 'Leave a Comment'
            "
            v-model="comment"
            required
          ...
          ></textarea>
          <button
            :class="!userInfo.id ? 'disabled-btn' : 'comment-btn'"
            type="submit"
            :disabled="!userInfo.id"
          >
            Post Comment
          ...
          <div v-for="comm in comments" :key="comm?._id" class="comment">
            <div class="info">
              <div class="user">
                <div class="avatar">
                  <span>{{
                    comm?.author?.username.charAt(0).toUpperCase() || "A"
                  }}</span>
                </div>
                <div class="name-date">
                  <h3>
                    {{ comm?.author?.username || "Anonymous" }}
                  </h3>
                  <span class="date">Commented {{ comm.createdAt }} ago</span>
                </div>
              </div>
              <div
                v-if="userInfo && userInfo.id === comm?.author?._id"
                @click="
                  showConfigCommentHandler({ id: comm._id, text: comm.body })
                "
                class="config"
              >
            ...
            <p>{{ comm.body }}</p>
            <div
              @click="showRepliesHandler(comm?._id, comm?.body)"
              class="reply"
            >
              <span
                >Reply
                <i class="fas fa-reply">
                  <small>( {{ comm?.replies?.length }} Replies )</small></i
                ></span
              >
            </div>
          </div>
